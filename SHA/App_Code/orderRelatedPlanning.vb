...
Partial Public Class orderRelatedPlanning
    ...
    Public Function preCheckBeforeOrdersImportation(ByRef continued As Boolean) As String
        ...
        If True Then
            ...
            Dim command As New SqlCommand("Select distinct Production_line From Esch_Sh_tbl_Lead_Time", conn)
            ...
            For i1 = 0 To arrayLine1.Count - 1
                If cmpSTR.IndexOf("@" & arrayLine1(i1) & "@") < 0 Then
                    continued = False
                    rrtMessage.Append("Production line ( <span style='color:red;'>" & arrayLine1(i1) & "</span> ) has not been set up in <a  href='../SCMrelated/currentLeadtime.aspx'> table Esch_Sh_tbl_Lead_Time.<a/>")
                    Exit For
                End If
            Next
            reader.Close()
            ...
            command.CommandText = "Select distinct int_line_no From Esch_Sh_tbl_orders"
            reader = command.ExecuteReader()
            compString.Clear()
            compString.Append("@")
            For i1 = 0 To arrayLine1.Count - 1
            ...
            Do While reader.Read()
                If cmpSTR.IndexOf("@" & reader("int_line_no") & "@") < 0 Then
                    continued = False
                    rrtMessage.Append("Production line (" & reader("int_line_no") & ") has not been set up in <a  href='../plansetting/PrdctnLinesAndBatchNoGroup.aspx'> table Esch_Sh_tbl_output_by_line_only.</a>")
                    Exit Do
                End If
            Loop
        ...
        If continued Then
            ...
            connParam.Open()
            Dim commandP As New SqlCommand("Select distinct int_line_no From Esch_Sh_tbl_LinesAndOwners", connParam)
            ...
            For i1 = 0 To arrayLine1.Count - 1
                If cmpSTRP.IndexOf("@" & arrayLine1(i1) & "@") < 0 Then
                    continued = False
                    rrtMessage.Append("No production line (" & arrayLine1(i1) & ") has not been set up in <a  href='../plansetting/LinesAndOwners.aspx'> table Esch_Sh_tbl_LinesAndOwners</a>")
                    Exit For
                End If
            Next
            readerP.Close()
    ...
    ' Esch_Sh_Esch_Sh_tbl_orders_new_revision_from_OPM ===> Esch_Sh_tbl_orders
    ...
    Public Sub NewRevisionOrderDataToMainOrderTable(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Dim command As New SqlCommand("SELECT txtFieldName FROM Esch_Sh_tbl_interface_mapping WHERE intImportOrderMapping > 0 ORDER BY intImportOrderMapping", conn)
        ...
        Dim dtUpdateFrom As SqlDataAdapter = New SqlDataAdapter("SELECT " & sqlSelect.ToString() & " FROM Esch_Sh_Esch_Sh_tbl_orders_new_revision_from_OPM", conn)

        sqlSelect.Append(" , txt_order_status,planned_production_qty,dat_finish_date,txt_order_type ")
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT " & sqlSelect.ToString() & " FROM Esch_Sh_tbl_orders ", conn)
    ...
    Public Sub assignLineToNewOrder(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_assign_line_by_order_qty", connParam)
        Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_assign_line_by_grade", connParam)
        Dim dtUpdateFrom2 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_Item_And_line_no_Priority_Sequence", conn)
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,int_line_no,dat_start_date,txt_item_no,txt_grade,planned_production_qty FROM Esch_Sh_tbl_orders WHERE (int_status_key = '" & newOrderStatus & "')", conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
        dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
        dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
        Dim dsAccess As DataSet = New DataSet
    ...
    Public Sub MOQappliedToNewOrder(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT groupName,headerName,relationOperator,conditionValue,remarkToBeAdded FROM Esch_Sh_tbl_MOQ", connParam)
        Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT DISTINCT groupName FROM Esch_Sh_tbl_MOQ", connParam)
        Dim dsAccess As DataSet = New DataSet
        ...
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,int_line_no,dat_start_date,txt_remark " & sqlSelect.ToString & " FROM  Esch_Sh_tbl_orders  WHERE (int_status_key = '" & newOrderStatus & "') ", conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
        dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
        dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
    ...
    Public Sub Preparation_For_Automatic_scheduling(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_VIP_lead_time ORDER BY DaysOfCommittedLeadtime DESC,VIPgroup ASC", connParam)  'sort the data in order to make the shortest lead time VIP prioritized
        Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT [VIPleadtimeTable].VIPgroup FROM (SELECT VIPgroup, avg(DaysOfCommittedLeadtime) AS leadtimeDesc FROM Esch_Sh_tbl_VIP_lead_time GROUP BY VIPgroup)  AS VIPleadtimeTable ORDER BY [VIPleadtimeTable].leadtimeDesc DESC", connParam)
        Dim dsAccess As DataSet = New DataSet
    ...
    Public Function reservedCapForVIP(ByRef conn As SqlConnection) As Long
        ...
        If CBool(valueOf("bnlReserveCapForVIP")) Then  'if you decide to reserve capacity for VIP customers
            ...
            Dim command As New SqlCommand("SELECT sum(ReservedCap) AS sumReservedCap FROM (SELECT Avg(ReservedCapPerWeek) as ReservedCap  From Esch_Sh_tbl_VIP_lead_time GROUP BY VIPgroup)", conn)
            Dim reader As SqlDataReader
            reader = command.ExecuteReader()
    ...
    ''' <summary>
    ''' Use to calculate leadtime per line per line group based on the setting in the table Esch_Sh_tbl_Lead_Time
    ''' </summary>
    Public Sub calculateLeadtime(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,int_line_no,dat_start_date,txt_auxiliary_code,int_status_key,txt_VIP,lng_VIP_lead_time,lng_AdvanceOfRevision FROM  Esch_Sh_tbl_orders  WHERE (int_line_no <> " & valueOf("intDummyLine") & " ) And (int_status_key Not In ('invoiced','cancelled')) And ((dat_start_date Is Null) Or (dat_start_date > " & dateSeparator & DateTime.Now & dateSeparator & "))", conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
        dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
        dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
        Dim dsAccess As DataSet = New DataSet
    ...
    Public Sub initiateCacheFromLeadtimeTable()
        ...
        If Cache("Leadtime") Is Nothing Then
            ...
            Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_Lead_Time", conn)

            Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT max(ReservedCapPerWeek) AS weeklyMax FROM Esch_Sh_tbl_VIP_lead_time Group By VIPgroup", connParam)
            ...
            dtUpdateFrom1.Fill(dsAccess, "Esch_Sh_tbl_VIP_lead_time")
            ...
            Dim reservedCapForVIPeveryWeek = (From vip In dsAccess.Tables("Esch_Sh_tbl_VIP_lead_time").AsEnumerable Select vip.Field(Of Integer)("weeklyMax")).Sum()
    ...
    Public Sub scheduleFreeSampleStartTime(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Try
            ...
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,dat_start_date,dat_etd,txt_remark  FROM  Esch_Sh_tbl_orders  WHERE (int_line_no <> " & valueOf("intDummyLine") & " ) And (int_status_key = '" & newOrderStatus & "') And ((txt_local_so Like 'FN%') Or (txt_local_so Like '%SMP%') Or (flt_sales_price = 0))", conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
            dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
            Dim dsAccess As DataSet = New DataSet
    ...
    Public Sub paymentTermsCheck(ByRef conn As SqlConnection, ByRef msg As String)
        ...
        Try
            ...
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,dat_start_date,txt_payment_status  FROM  Esch_Sh_tbl_orders  WHERE (int_line_no <> " & valueOf("intDummyLine") & " ) And (int_status_key = '" & newOrderStatus & "') And Not ((txt_local_so Like 'FN%') Or (txt_local_so Like '%SMP%') Or (flt_sales_price = 0)) And txt_payment_term In " & paymentTerms, conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
            dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
            Dim dsAccess As DataSet = New DataSet
    ...
    ''' <summary>
    ''' Need resolve the updated lead time back to table Esch_Sh_tbl_Lead_Time
    ''' </summary>
    Public Sub resolveLeadtimeBackToTable(ByRef conn As SqlConnection, ByRef msg As String)

        Try

            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT *  FROM  Esch_Sh_tbl_Lead_Time", conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
            dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
