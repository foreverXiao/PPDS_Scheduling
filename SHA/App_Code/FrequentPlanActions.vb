...
Partial Public Class FrequentPlanActions
    ...
    Public Function Planned_production_qty(ByRef conn As SqlConnection) As String
        ...
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,flt_unallocate_qty,planned_production_qty,int_line_no FROM Esch_Sh_tbl_orders WHERE ((dat_start_date > " & dateSeparator & DateTime.Now().AddHours(2) & dateSeparator & ") AND ( planned_production_qty <> flt_unallocate_qty) And (int_status_key Not In ('invoiced','cancelled'))) Or (planned_production_qty Is Null)", conn)
    ...
    Public Function assignValueToBlankPrice(ByRef conn As SqlConnection) As String
        ...
        'planned production quantity will not change if the start time is within 16 hours
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,flt_sales_price FROM Esch_Sh_tbl_orders WHERE (flt_sales_price Is Null)", conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
        dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
        dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
        Dim dsAccess As DataSet = New DataSet
    ...
    Public Function calculateRSD(ByRef conn As SqlConnection, Optional ByVal filterCondition As String = " (int_status_key Not In ('invoiced','cancelled','old')) ") As String
        ...
        Try
            ...
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,txt_item_no,dat_rdd,dat_etd,txt_currency,txt_destination,txt_ship_method,txt_end_user FROM Esch_Sh_tbl_orders WHERE " & filterCondition, conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
    ...
    Public Function calculateRSD1(ByRef conn As SqlConnection, ByRef dtRows() As DataRow) As String
        ...
        Try
            ...
            Dim dtUpdateFrom As SqlDataAdapter = New SqlDataAdapter("SELECT txt_currency,txt_destination,txt_ship_method,flt_transit FROM Esch_Sh_tbl_transit", connParam)
    ...
    Public Function finishTime_exPlantDate_Span(ByRef conn As SqlConnection, ByRef hasException As Boolean, Optional ByVal whereClauseCondition As String = " ") As String
        ...
        Try
            ...
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_orders WHERE (int_status_key Not In ('invoiced','cancelled')) And (dat_start_date > " & dateSeparator & DateTime.Today.AddDays(-7) & dateSeparator & ") " & whereClauseCondition, conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
    ...
    Public Function finishTime_exPlantDate_Span1(ByRef conn As SqlConnection, ByRef dtRows() As DataRow, ByRef hasException As Boolean, Optional ByVal limitedLine As String = " ", Optional ByVal whereClauseCondition As String = " ") As String
        ...
        Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_output_by_line_by_grade" & limitedLine, connParam)
        Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_output_by_line_by_item" & limitedLine, connParam)
        Dim dtUpdateFrom3 As SqlDataAdapter = New SqlDataAdapter("SELECT *  FROM Esch_Sh_tbl_output_by_line_only", connParam)

        Dim dtUpdateFrom4 As SqlDataAdapter = New SqlDataAdapter("SELECT *  FROM Esch_Sh_tbl_Validity", conn)
        ...
        'delete records in table Esch_Sh_tbl_Validity
        ...
        For Each a As DataRow In dtRows
            ...
            If byItem.Count > 0 Then
                If byItem(0).Item("int_rate") > 0 Then
                    ...
                    continues = False
                Else  'record some special issue in table Esch_Sh_tbl_Validity due to no suitable capacity setting on this item
            ...
            If continues Then
                ...
                If byGrade.Count > 0 Then
                    If byGrade(0).Item("int_rate") > 0 Then
                        ...
                        continues = False
                    Else  'record some special issue in table Esch_Sh_tbl_Validity due to no suitable capacity setting on this grade
            ...
            If continues Then
                ...
                If byLineonly.Count > 0 Then
                    If byLineonly(0).Item("int_rate") > 0 Then
                        ...
                        continues = False
                    Else  'record some special issue in table Esch_Sh_tbl_Validity due to no default capacity setting on this production line
                        Dim vldtyNew As DataRow = dsAccess.Tables("validity").NewRow
    ...
    Public Function additionDaysOnExplantDate(ByRef conn As SqlConnection, ByRef updateTo As DataTable, Optional ByVal dataviewRowstateInt As Integer = DataViewRowState.Added) As String
        ...
        'quality concern =================================================
        Dim dtUpdateFrom6 As SqlDataAdapter = New SqlDataAdapter("SELECT *  FROM Esch_Sh_tbl_qualityConcern", connParam)
        ...
        Dim dtUpdateFrom5 As SqlDataAdapter = New SqlDataAdapter("SELECT *  FROM Esch_Sh_tbl_currencyCustoms order by extraDays DESC", connParam)
    ...
    Public Function PackageOrMiscellaneous(ByRef conn As SqlConnection) As String
        ...
        Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT groupName,headerName,relationOperator,conditionValue,remarkToBeAdded FROM Esch_Sh_tbl_remarkPerCondition", connParam)
        Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT DISTINCT groupName FROM Esch_Sh_tbl_remarkPerCondition", connParam)
        Dim dsAccess As DataSet = New DataSet
        ...
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,int_line_no,dat_start_date,txt_remark " & sqlSelect.ToString & " FROM  Esch_Sh_tbl_orders  WHERE (int_status_key = 'NEWnew') ", conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
        dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
        dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
    ...
    Public Overloads Function AssignScrewDieAndFDA(ByRef conn As SqlConnection, Optional ByVal filterCondition As String = " (int_status_key = '" & newOrderStatus & "') ") As String
        ...
        Try
            ...
            Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_screw_die_FDA_by_grade", connParam)
            Dim dtUpdateFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_screw_die_by_item", connParam)
            Dim dsAccess As DataSet = New DataSet
            ...
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,txt_grade,txt_item_no,txt_FDA,txt_process_technics  FROM  Esch_Sh_tbl_orders  WHERE " & filterCondition, conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
            dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
            ...
            dtUpdateTo.Fill(dsAccess, "UpdateTo")
            Dim Esch_Sh_tbl_orders As DataTable = dsAccess.Tables("UpdateTo")
            Dim keys(0) As DataColumn
            keys(0) = dsAccess.Tables("UpdateTo").Columns("txt_order_key")
            dsAccess.Tables("UpdateTo").PrimaryKey = keys
            ...
            For Each a As DataRow In dsAccess.Tables("byGrade").Rows
                Dim b() As DataRow = Esch_Sh_tbl_orders.Select("txt_grade = '" & a.Item("txt_grade") & "'")
            ...
            For Each a As DataRow In dsAccess.Tables("byItem").Rows
                Dim b() As DataRow = Esch_Sh_tbl_orders.Select("txt_item_no = '" & a.Item("txt_item_no") & "'")
                For Each c As DataRow In b  'update remark once we found any order matching the criteria
                    c.Item("txt_process_technics") = a.Item("txt_screw").ToString & " " & a.Item("txt_die").ToString
                Next
            Next
    ...
    Public Function UpdateOrderCompletionPercentage(ByRef conn As SqlConnection) As String
        ...
        Try
            ...
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,flt_actual_qty_man,flt_actual_completed,planned_production_qty  FROM  Esch_Sh_tbl_orders  WHERE dat_start_date > " & dateSeparator & DateTime.Today.AddDays(-7).Date & dateSeparator & "", conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
    ...
    Public Function deleteUponCondition(ByRef conn As SqlConnection, Optional ByVal condition As String = "int_status_key  ='cancelled'") As Integer
        ...
        'only focus on those orders with RDD not earlier than one week ago
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key  FROM  Esch_Sh_tbl_orders  WHERE  " & condition, conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
    ...
    Public Sub productionLinesOwnedByValiduser()
        ...
        If CacheFrom("Ls") Is Nothing Then 'see if we need refresh all the lines which are owned by valid user
            ...
            Dim connParam As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings(dbConnForParam).ConnectionString)
            connParam.Open()
            Dim command As New SqlCommand("SELECT int_line_no,validuser FROM Esch_Sh_tbl_LinesAndOwners  ", connParam)
