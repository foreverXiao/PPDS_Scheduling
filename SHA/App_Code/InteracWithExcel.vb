...
Partial Public Class InteracWithExcel
    ...
    ''' <summary>
    ''' get a full list of column name in table Esch_Sh_tbl_orders  in order to do some checking on data validity
    ''' </summary>
    Protected Function getColumnNameOf_Esch_Sh_tbl_orders() As String

        Dim connstr As String = ConfigurationManager.ConnectionStrings(dbConnectionName).ConnectionString
        Dim conn As SqlConnection = New SqlConnection(connstr)
        Dim dtUpdateFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_Sh_tbl_orders WHERE txt_order_key = 'Null'", conn)
