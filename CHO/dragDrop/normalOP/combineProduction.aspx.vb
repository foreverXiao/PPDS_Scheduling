...
Partial Class dragDrop_normalOP_combineProduction
    ...
    Protected Sub prdctnOrdrs_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles prdctnOrdrs.Click
        ...
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_similar_item_combination", conn)
        ...
        Dim dtFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_similar_grade_for_combination", connParam)
        Dim dtTableFrom1 As DataTable = New DataTable()
        dtFrom1.Fill(dtTableFrom1)



        For Each dtrow0 As DataRow In dtTableFrom0.Select(Nothing, "[txt_item_no] ASC, [dat_start_date] ASC, [int_line_no] ASC") 'get order line data from table Esch_CQ_tbl_orders
    ...
    Protected Sub bTdb_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bTdb.Click
        ...
        Dim dtFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_orders ", conn)
        'Dim dtFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT txt_remark,planned_production_qty,int_line_no,txt_lot_no,dat_start_date,dat_finish_date,txt_order_key,flt_working_hours,int_change_over_time,dat_new_explant,txt_item_no,txt_grade,int_span,dat_etd,txt_end_user FROM Esch_CQ_tbl_orders ", conn)
        ...
        Dim dtAdapter As SqlDataAdapter = New SqlDataAdapter("SELECT planned_production_qty,int_line_no,txt_lot_no,dat_start_date,txt_remark,txt_order_key FROM Esch_CQ_tbl_similar_item_combination", conn)
        ...
        ''update back to table Esch_CQ_tbl_orders
    ...
    Protected Sub prdctnOrdrs2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles prdctnOrdrs2.Click
        ...
        Dim dtFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT txt_item_no,planned_production_qty,int_line_no,txt_lot_no,dat_start_date,dat_finish_date,txt_currency,dat_etd,dat_rdd,txt_remark,txt_grade,txt_color,txt_order_key,int_status_key FROM Esch_CQ_tbl_orders " & sqlWhereClause.ToString(), conn)
        Dim dtTableFrom0 As DataTable = New DataTable()
        dtFrom0.Fill(dtTableFrom0)
        'how many records to be inserted into Esch_CQ_tbl_similar_item_combination
        Dim recordsCount As Integer = 0
        ...
        For Each dtrow0 As DataRow In dtTableFrom0.Select(Nothing, "[txt_item_no] ASC, [dat_etd] ASC") 'get order line data from table Esch_CQ_tbl_orders
            Dim newRow As DataRow = dtTableUpdateTo.NewRow()
        ...
        SDS1.SelectCommand = "SELECT * FROM Esch_CQ_tbl_similar_item_combination ORDER BY txt_item_no_similar ASC,txt_item_no ASC,dat_etd ASC, dat_start_date ASC , int_line_no ASC "
