...
Partial Class dragDrop_normalOP_MTI
    ...
    Public Overrides Function dataValidityCheck(ByRef dtTbl As DataTable) As String
        ...
        Try
            For Each rowexcel As DataRow In dtTbl.Rows
                ...
                ' Add check txt_currency  and  txt_package_code and txt_ship_method  ----Gary Xu 20150108
                If String.IsNullOrEmpty(rowexcel("txt_currency")) Then
                    msstring = "Empty value in field txt_currency is not allowed."
                End If
    ...
    Protected Sub LV1_ItemUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ListViewUpdateEventArgs) Handles LV1.ItemUpdating
        ...
        ' Add check txt_currency  and  txt_package_code and txt_ship_method  ----Gary Xu 20150108
        If String.IsNullOrEmpty(e.NewValues("txt_currency")) Then
            messageText.Append("Empty value in field txt_currency is not allowed.")
        End If
    ...
    ''' <summary>
    ''' add MTI orders to table Esch_CQ_tbl_orders
    ''' </summary>
    Protected Sub bckToDB_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles bckToDB.Click
        ...
        If String.IsNullOrEmpty(userName) Then
            ...
            Dim dtUpdateTo0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_orders WHERE txt_order_key = ''", conn)
            ...
            Dim dtUpdateTo1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_MTI_add WHERE decideToAdd = 1 ", conn)
            ...
            For Each r1 As DataRow In dtTbl1.Rows
                ...
                'add below three items in below ---Gary Xu 20150108
                newR.Item("txt_currency") = r1.Item("txt_currency")
                newR.Item("txt_package_code") = r1.Item("txt_package_code")
                newR.Item("txt_ship_method") = r1.Item("txt_ship_method")
                ...
                r1.Item("decideToAdd") = False 'use to mark that this item has been added to table Esch_CQ_tbl_orders
