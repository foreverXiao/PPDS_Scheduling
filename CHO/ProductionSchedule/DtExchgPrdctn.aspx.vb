...
Partial Class ProductionSchedule_DtExchgPrdctn
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Cache("production") Is Nothing Then 'need refresh data for planning period parameter
            ...
            'get the lastest data from table Esch_CQ_tbl_plnprmtr to preset some planning parameter

            Dim command1 As New SqlCommand("Select paramname, paramvalue From Esch_CQ_tbl_plnprmtr where category = 'production'", connParam)
        ...
        If actionRequested = "generateExcelFile" Then
            If True OrElse CacheFrom("sso") Is Nothing Then
                ...
                ' " ((Select txt_order_key,int_line_no,dat_start_date,dat_finish_date,txt_lot_no,int_span,planned_production_qty,flt_unallocate_qty,flt_order_qty,txt_color,txt_item_no,dat_etd,txt_VIP,txt_process_technics,txt_FDA,txt_remark,txt_payment_status From Esch_CQ_tbl_orders  Where CAST(int_line_no AS VARCHAR(5)) <> '" & valueOf("intDummyLine") & "' And  ((dat_start_date  between " & dateSeparator & startD & dateSeparator & " And " & dateSeparator & DateAdd("h", daysFromStart * 24, startD) & dateSeparator & ") Or (dat_finish_date  between " & dateSeparator & startD & dateSeparator & " And " & dateSeparator & DateAdd("h", daysFromStart * 24, startD) & dateSeparator & "))) as table1 Left Join (select txt_order_key,txt_pull_screw From Esch_CQ_tbl_prductn_prmtr) as table2 On (table1.txt_order_key = table2.txt_order_key)) Order by int_line_no,table1.dat_finish_date ASC, table1.dat_start_date ASC "
                ...
                Dim lotList As String = " SELECT DISTINCT txt_lot_no FROM Esch_CQ_tbl_orders  Where NOT (txt_lot_no IS NULL) AND txt_lot_no <> '' AND " & noDummyLineAndSpecificStartFinishTime
                query1 = " Select txt_order_key,int_line_no,dat_start_date,dat_finish_date,txt_lot_no,int_span,planned_production_qty,flt_unallocate_qty,flt_order_qty,txt_color,txt_item_no,dat_etd,txt_VIP,txt_process_technics,txt_FDA,txt_remark,txt_payment_status FROM  Esch_CQ_tbl_orders WHERE ( CAST(int_line_no AS VARCHAR(5)) <> '" & valueOf("intDummyLine") & "' AND txt_lot_no IN (" & lotList & ")) OR " & noDummyLineAndSpecificStartFinishTime
                query1 = " SELECT table1.txt_order_key,int_line_no,dat_start_date,dat_finish_date,txt_lot_no,int_span,planned_production_qty,flt_unallocate_qty,flt_order_qty,txt_color,txt_item_no,dat_etd,txt_VIP,txt_process_technics,txt_FDA,txt_remark,txt_payment_status,txt_pull_screw FROM ( (" & query1 & ") AS table1 LEFT JOIN Esch_CQ_tbl_prductn_prmtr ON (table1.txt_order_key = Esch_CQ_tbl_prductn_prmtr.txt_order_key)) Order by int_line_no, " & DAT_START_DATE & "  ASC, " & DAT_FINISH_DATE & " ASC"
    ...
    Public Sub listOutDifferencesBetweenScheduleUpdates(ByRef counter As Integer)
        ...
        Dim dtAdapterFrom As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,int_line_no,txt_item_no,txt_lot_no,planned_production_qty,dat_start_date FROM Esch_CQ_tbl_orders WHERE (dat_start_date > " & dateSeparator & Today.AddMonths(-1) & dateSeparator & ") And (dat_start_date < " & dateSeparator & Today.AddMonths(1) & dateSeparator & ")", conn)
        Dim dtTableFrom As DataTable = New DataTable
        dtAdapterFrom.Fill(dtTableFrom)

        Dim dtAdapterTo1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_differencesInBetweens ", conn)
