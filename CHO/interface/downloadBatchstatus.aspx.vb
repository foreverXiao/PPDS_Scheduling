...
Partial Class interface_downloadBatchstatus
    ...
    Protected Sub batchstatus_FromOPMftpServer(ByRef ftpMsg As String)
        ...
        Try
            ...
            'pass the information of batch status from table Esch_CQ_tbl_BatchSts_from_OPM  to table Esch_CQ_tbl_orders
            If continues Then
                Dim err2 As String = String.Empty
                msge.Append(batchStatusFromTableTo_Esch_CQ_tbl_orders(err2))
    ...
    ''' <summary>
    ''' after download text files about batch status, extracting data from these files and import them into access table Esch_CQ_tbl_BatchSts_from_OPM 
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub batchStatusFromOPMtoTable(ByRef errorM As String)
        ...
        Dim command As New SqlCommand("SELECT txtFieldName FROM Esch_CQ_tbl_interface_mapping WHERE intImpBatchstatusMapping > 0 ORDER BY intImpBatchstatusMapping", conn)
        Dim reader As SqlDataReader = command.ExecuteReader()
        Dim mapping1() As String = {String.Empty}, icount As Integer = 0
        Dim btchstsTable As String = "Esch_CQ_tbl_BatchSts_from_OPM"
        While reader.Read()
    ...
    ''' <summary>
    ''' transfer batch status from table to table Esch_CQ_tbl_BatchSts_from_OPM  to table Esch_CQ_tbl_orders
    ''' </summary>
    ''' <param name="errorMsg"></param>
    ''' <returns></returns>
    Public Function batchStatusFromTableTo_Esch_CQ_tbl_orders(ByRef errorMsg As String) As String
        ...
        Dim command As SqlCommand = New SqlCommand("SELECT [txt_batch_status],[txt_meaning]  FROM  [Esch_CQ_tbl_batch_status_meaning] ", connParam)
        ...
        Dim dtUpdateFrom As SqlDataAdapter = New SqlDataAdapter("Select * From Esch_CQ_tbl_BatchSts_from_OPM ", conn)

        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,txt_lot_no,txt_batch_status,txt_actual_line_no,dat_actual_start,dat_actual_finish,flt_actual_qty FROM  Esch_CQ_tbl_orders  WHERE (txt_lot_no Is Not Null) And ( int_status_key <> 'cancelled' )   ORDER BY  txt_lot_no ASC ", conn)
    ...
    Protected Sub GetBatchStatusFromQA_eColor_SQLserver(ByVal startPoint As DateTime, ByRef errMsg As String, ByRef nrmlMsg As String)
        ...
        Try
            ...
            Dim conn As SqlConnection = New SqlConnection(connstr)
            Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,int_line_no,txt_lot_no,dat_start_date,dat_finish_date,planned_production_qty,dat_start_from_qa,flt_actual_completed,flt_actual_qty_man FROM Esch_CQ_tbl_orders WHERE  ( txt_lot_no Is Not Null )  And ( int_status_key <> 'cancelled' ) And ( (dat_start_date between " & dateSeparator & startPoint & dateSeparator & " And " & dateSeparator & now1.AddDays(3) & dateSeparator & ")  Or (dat_finish_date  between " & dateSeparator & startPoint & dateSeparator & " And " & dateSeparator & now1.AddDays(3) & dateSeparator & ") )  ORDER BY txt_lot_no ASC,dat_start_date ASC", conn)
            Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtUpdateTo)
            dtUpdateTo.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
            dtUpdateTo.InsertCommand = cmdbAccessCmdBuilder.GetInsertCommand()
            Dim dsAccess As DataSet = New DataSet
