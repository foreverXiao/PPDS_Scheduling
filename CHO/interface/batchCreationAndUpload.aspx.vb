...
Partial Class interface_batchCreationAndUpload
    ...
    Protected Sub CreateBatch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles CreateBatch.Click
        ...
        'get data from table Esch_CQ_tbl_batch_no_group_and_batch_rules
        Dim connParam As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings(dbConnForParam).ConnectionString)
        Dim dtAdapterParam As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_batch_no_group_and_batch_rules", connParam)
        ...
        'get data from table Esch_CQ_tbl_BatchNO
        Dim conn As SqlConnection = New SqlConnection(SDS1.ConnectionString)
        Dim dtAdapter As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_BatchNO ", conn)
    ...
    Protected Sub prdctnOrdrs_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles prdctnOrdrs.Click
        ...
        Dim dtUpdateTo As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_BatchNO", conn)
        ...
        If noLineWasSelected Then
            sqlWhereClause.Append("'-1') ") 'assign a false production line  number to the field to delete all the records in table Esch_CQ_tbl_BatchNO
        Else
        ...
        Dim dtFrom0 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_orders " & sqlWhereClause.ToString() & " ORDER BY int_line_no,txt_item_no,dat_start_date ", conn)
        Dim dtTableFrom0 As DataTable = New DataTable()
        dtFrom0.Fill(dtTableFrom0)
        'how many records to be inserted into Esch_CQ_tbl_BatchNO
        ...
        Dim dtBLP As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_BLP_new ", connParam)
        ...
        Dim dtFrom2 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_LinesAndOwners ", connParam)
        ...
        For Each dtrow0 As DataRow In dtTableFrom0.Rows 'get order line data from table Esch_CQ_tbl_orders
            ...
            'insert  txt_line_description and txt_formula_version into table Esch_CQ_tbl_BatchNO according to the information in table  Esch_CQ_tbl_LinesAndOwners
        ...
        'update batch no group based on the conditions set in table Esch_CQ_tbl_production_lines_and_batch_no_group
        Dim dtFrom1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_production_lines_and_batch_no_group", connParam)
        ...
        Catch ex As Exception
            msgRtrn.AppendLine("<div style = 'color:red;'> update batch group name to Esch_CQ_tbl_BatchNO: " & ex.Message & "</div>")
        End Try




        'write data back to table Esch_CQ_tbl_BatchNO
    ...
    Protected Sub gnrtFlndFTP_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles gnrtFlndFTP.Click
        ...
        'get data from table Esch_CQ_tbl_MTI_List 
        Dim dtAdapterMTIlist As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_MTI_List WHERE dat_expiration_date >= " & dateSeparator & Today & dateSeparator & " AND dat_effective_date <= " & dateSeparator & Today & dateSeparator, connParam)
        ...
        If Not errorOccurred Then
            ...
            If returnMessageAfterFTPoperation = "true" Then
                ...
                Dim dtAdapterOrders As SqlDataAdapter = New SqlDataAdapter("SELECT txt_order_key,txt_lot_no,int_formula_version FROM Esch_CQ_tbl_orders", conn)
                ...
                'update table Esch_CQ_tbl_BatchNO
                dtAdapter.Update(dtTable)




                'update table Esch_CQ_tbl_batch_no_group_and_batch_rules to get the latest sequence of Lot No. (Batch No.)
                'update field txt_current_no based on the field value of txt_last_no
                For Each a As DataRow In dtTableParam.Rows
    ...
    Protected Sub cbAllOrders_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbAllOrders.CheckedChanged
        Dim conn As SqlConnection = New SqlConnection(SDS1.ConnectionString)
        Dim dtAdapter As SqlDataAdapter = New SqlDataAdapter("SELECT bnl_Y_or_N,txt_lot_no,txt_order_key FROM Esch_CQ_tbl_BatchNO", conn)
        Dim cmdbAccessCmdBuilder As New SqlCommandBuilder(dtAdapter)
        dtAdapter.UpdateCommand = cmdbAccessCmdBuilder.GetUpdateCommand()
        Dim dtTable As DataTable = New DataTable()
        dtAdapter.Fill(dtTable)
    ...
    Protected Sub updateLineItemSequenceTable()
        ...
        Dim dtAdapterFrom As SqlDataAdapter = New SqlDataAdapter("SELECT COUNT(bnl_Y_or_N) AS countNum,txt_item_no,int_line_no FROM Esch_CQ_tbl_BatchNO WHERE bnl_Y_or_N > 0 GROUP BY txt_item_no,int_line_no ", conn)
        Dim dtTableFrom As DataTable = New DataTable
        dtAdapterFrom.Fill(dtTableFrom)

        Dim dtAdapterTo1 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_Item_And_line_no_historical_data ", conn)
        ...
        Dim dtAdapterTo2 As SqlDataAdapter = New SqlDataAdapter("SELECT * FROM Esch_CQ_tbl_Item_And_line_no_Priority_Sequence ", conn)
        ...
        'iterate through dataTable Esch_CQ_tbl_BatchNO to update table Esch_CQ_tbl_Item_And_line_no_historical_data and  Esch_CQ_tbl_Item_And_line_no_Priority_Sequence
